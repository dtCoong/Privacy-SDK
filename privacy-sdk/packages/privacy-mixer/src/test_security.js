...
console.log(
  "=== BẮT ĐẦU SECURITY TEST SUITE CHO RINGSIG.JS (FINAL ATTEMPT) ==="
);
...
async function runTests() {
  ...
  try {
    signature = await RingSignature.createRingSignature(
      message,
      publicKeysHex,
      signer.sk,
      signerIndex
    ...
    const isTamperedMsgValid = await RingSignature.verifyRingSignature(
      "Hacker",
      signature,
      publicKeysHex
    );
    ...
    const isFakeRingValid = await RingSignature.verifyRingSignature(
      message,
      signature,
      fakeRing
    );
    ...
    const isMalformedValid = await RingSignature.verifyRingSignature(
      message,
      fakeSig,
      publicKeysHex
    );
  ...
  try {
    if (!signature) {
      ...
      const sig2 = await RingSignature.createRingSignature(
        "Transaction 2",
        publicKeysHex,
        signer.sk,
        signerIndex
      ...
      const sig3 = await RingSignature.createRingSignature(
        "Tx 3",
        publicKeysHex,
        otherSigner.sk,
        0
  ...
  try {
    const sigA = await RingSignature.createRingSignature(
      "Test",
      publicKeysHex,
      ringKeys[0].sk,
      0
    );
    const sigB = await RingSignature.createRingSignature(
      "Test",
      publicKeysHex,
      ringKeys[1].sk,
      1
