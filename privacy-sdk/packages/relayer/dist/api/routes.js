...
function createRoutes(relayService, queue) {
    ...
    router.get('/info', async (req, res) => {
        try {
            ...
            logger_1.default.error('Failed to get info', { error: error.message });
            res.status(500).json({
                success: false,
                error: error.message
            });
    ...
    router.post('/api/v1/submit', async (req, res) => {
        try {
            ...
            logger_1.default.info('Received relay request', { request, priority });
            // Enqueue transaction
            ...
            logger_1.default.error('Failed to submit', { error: error.message });
            res.status(500).json({
                success: false,
                error: error.message
            });
    ...
    router.get('/api/v1/status/:txId', async (req, res) => {
        try {
            ...
            logger_1.default.error('Failed to get status', { error: error.message });
            res.status(500).json({
                success: false,
                error: error.message
            });
    ...
    // Get anonymity set
    router.get('/api/v1/anonymity-set', async (req, res) => {
        try {
            ...
            logger_1.default.error('Failed to get anonymity set', { error: error.message });
            res.status(500).json({
                success: false,
                error: error.message
            });
    ...
    // Queue statistics
    router.get('/api/v1/queue/stats', (req, res) => {
        try {
            const stats = queue.getStats();
            res.json({
                success: true,
    ...
    router.get('/admin/queue/next', (req, res) => {
        try {
            ...
            logger_1.default.error('Failed to get next transaction', { error: error.message });
            res.status(500).json({
                success: false,
                error: error.message
            });
    ...
    router.post('/admin/nonce/reset', async (req, res) => {
        try {
            relayService.resetNonce();
            const newNonce = await relayService.getNonce();
            logger_1.default.info('Nonce reset', { newNonce });
            res.json({
                success: true,
            ...
            logger_1.default.error('Failed to reset nonce', { error: error.message });
            res.status(500).json({
                success: false,
                error: error.message
            });
